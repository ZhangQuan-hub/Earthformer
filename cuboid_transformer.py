...
from .CuboidSelfAttentionLayer import CuboidSelfAttentionLayer
from .utils import (
...
class PosEmbed(nn.Module):
    ...
    def __init__(self, embed_dim, maxT, maxH, maxW, typ='t+h+w'):
        ...
        输入：x → [B, T, H, W, C]
        生成索引：t_idx, h_idx, w_idx
        查表获取位置编码：
            "t+h+w"：分别查表并reshape广播
            "t+hw"：空间位置整体查表
        加到输入特征上：x + pos_embed
        输出：[B, T, H, W, C]（带位置信息）
        """
    ...
    def forward(self, x, x2=None):
        ...
        x
            Shape (B, T, H, W, C)
        x2
            Shape (B, T, H, W, C) - 可选，如果提供则对 x2 也进行位置编码

        Returns
        -------
        out
            Return the x + positional embeddings
            If x2 is provided, also return x2 + positional embeddings
        """
        _, T, H, W, _ = x.shape
        ...
        if self.typ == 't+h+w':
            pos_embed = self.T_embed(t_idx).reshape(T, 1, 1, self.embed_dim)\
            ...
            pos_embed = self.T_embed(t_idx).reshape(T, 1, 1, self.embed_dim) + self.HW_embed(spatial_idx)
        else:
            raise NotImplementedError
        ...
        if self.use_inter_ffn:
            if self.use_global_vector and global_vectors is not None:
                for idx, (attn, ffn) in enumerate(zip(self.attn_l, self.ffn_l)):
                    if self.checkpoint_level >= 2 and self.training:
                        x_out, x2_out, global_vectors_out = checkpoint.checkpoint(attn, x, x2, global_vectors)
                    else:
                        x_out, x2_out, global_vectors_out = attn(x, x2, global_vectors)
                    x = x + x_out
                    x2 = x2 + x2_out
                    ...
                    if self.checkpoint_level >= 1 and self.training:
                        ...
                        x2 = checkpoint.checkpoint(ffn, x2)
                ...
                return x, x2, global_vectors
            else:
                for idx, (attn, ffn) in enumerate(zip(self.attn_l, self.ffn_l)):
                    if self.checkpoint_level >= 2 and self.training:
                        x_out, x2_out = checkpoint.checkpoint(attn, x, x2)
                    else:
                        x_out, x2_out = attn(x, x2)
                    x = x + x_out
                    x2 = x2 + x2_out
                    if self.checkpoint_level >= 1 and self.training:
                        x = checkpoint.checkpoint(ffn, x)
            ...
            if self.use_global_vector and global_vectors is not None:
                ...
                if self.checkpoint_level >= 1 and self.training:
                    ...
                    x2 = checkpoint.checkpoint(self.ffn_l[0], x2)
                    if self.use_global_vector_ffn:
...
class CuboidTransformerEncoder(nn.Module):
    ...
    def forward(self, x, x2, global_vectors=None):
        ...
        x2
            Shape (B, T, H, W, C)
        global_vectors
            Shape (B, num_global, global_dim) - 可选

        Returns
        -------
        out_x, out_x2
            A list of tensors from the bottom layer to the top layer of the encoder for x and x2 respectively
        global_mem_out
            Optional
        """
        B, T, H, W, C_in = x.shape
...
class CuboidTransformerDecoder(nn.Module):
    ...
    def forward(self, x, mem_l, mem_l2, mem_global_vector_l=None):
        """
        ...
        mem_l
            A list of memory tensors for x
        mem_l2
            A list of memory tensors for x2
        mem_global_vector_l
            A list of global vectors (optional)
        ...
        for i in range(self.num_blocks - 1, -1, -1):
            ...
            if not self.use_first_self_attn and i == self.num_blocks - 1:
                ...
                if i >= self.cross_start:
                    # 使用交叉注意力，x 与 mem_l[i] 和 mem_l2[i] 进行交叉注意力
                    x = self.cross_blocks[i - self.cross_start][0](x, mem_l[i], mem_global_vector)
                    x = self.cross_blocks[i - self.cross_start][0](x, mem_l2[i], mem_global_vector)
                for idx in range(self.depth[i] - 1):
                    if self.use_self_global:  # in this case `mem_global_vector` is guaranteed to be not None
                        if self.self_update_global:
                            x, _, mem_global_vector = self.self_blocks[i][idx](x, x, mem_global_vector)
                        else:
                            x, _, _ = self.self_blocks[i][idx](x, x, mem_global_vector)
                    else:
                        x, _ = self.self_blocks[i][idx](x, x)  # 传递 x 作为 x2 的虚拟值
                    if i >= self.cross_start:
                        x = self.cross_blocks[i - self.cross_start][idx + 1](x, mem_l[i], mem_global_vector)
                        x = self.cross_blocks[i - self.cross_start][idx + 1](x, mem_l2[i], mem_global_vector)
            else:
                for idx in range(self.depth[i]):
                    ...
                    if i >= self.cross_start:
                        ...
                        x = self.cross_blocks[i - self.cross_start][idx](x, mem_l2[i], mem_global_vector)
            # Upsample
            if i > 0:
...
class InitialStackPatchMergingEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        # self.conv_in_1 = nn.Conv2d(kernel_size=3,padding=1,in_channels=in_dim,out_channels=in_dim)
        # self.conv_in_2 = nn.Conv2d(kernel_size=3,padding=1,in_channels=in_dim,out_channels=in_dim)
        self.conv_in = nn.Conv2d(kernel_size=1,padding=0,in_channels=in_dim*2,out_channels=in_dim)
    ...
    def forward(self, x, x2):
        ...
        x2 --> [K x Conv2D] --> PatchMerge --> ... --> [K x Conv2D] --> PatchMerge
        ...
        -------
        out_x, out_x2
            Shape (B, T, H_new, W_new, C_out) for both outputs

        """
        # 对 x 进行处理
        B, T, H, W, C = x.shape
        x = x.permute(0, 1, 4, 2, 3)  # [batch,sequence_length,channel,height,width]
        B, T, C, H, W = x.shape
        x = x.reshape(B*T, C, H, W)
        
        # 对 x2 进行处理
        B2, T2, H2, W2, C2 = x2.shape
        x2 = x2.permute(0, 1, 4, 2, 3)  # [batch,sequence_length,channel,height,width]
        B2, T2, C2, H2, W2 = x2.shape
        x2 = x2.reshape(B2*T2, C2, H2, W2)
        
        # 对 x 进行多层处理
        for i, (conv_block, patch_merge) in \
                enumerate(zip(self.conv_block_list, self.patch_merge_list)):
            # 将 x 重新转换为 5D 张量进行处理
            B_curr, C_curr, H_curr, W_curr = x.shape
            T_curr = B_curr // B  # 恢复时间维度
            x = x.reshape(B, T_curr, H_curr, W_curr, C_curr)

            if self.num_conv_per_merge_list[i] > 0:
                x = x.reshape(B * T_curr, H_curr, W_curr, C_curr).permute(0, 3, 1, 2)
                x = conv_block(x).permute(0, 2, 3, 1).reshape(B, T_curr, H_curr, W_curr, -1)
            x = patch_merge(x)
            # 将 x 重新转换为 4D 张量用于下一次循环
            B_new, T_new, H_new, W_new, C_new = x.shape
            x = x.reshape(B * T_new, C_new, H_new, W_new)
            
        # 对 x2 进行多层处理
        for i, (conv_block, patch_merge) in \
                enumerate(zip(self.conv_block_list, self.patch_merge_list)):
            # 将 x2 重新转换为 5D 张量进行处理
            B2_curr, C2_curr, H2_curr, W2_curr = x2.shape
            T2_curr = B2_curr // B2  # 恢复时间维度
            x2 = x2.reshape(B2, T2_curr, H2_curr, W2_curr, C2_curr)

            if self.num_conv_per_merge_list[i] > 0:
                x2 = x2.reshape(B2 * T2_curr, H2_curr, W2_curr, C2_curr).permute(0, 3, 1, 2)
                x2 = conv_block(x2).permute(0, 2, 3, 1).reshape(B2, T2_curr, H2_curr, W2_curr, -1)
            x2 = patch_merge(x2)
            # 将 x2 重新转换为 4D 张量用于下一次循环
            B2_new, T2_new, H2_new, W2_new, C2_new = x2.shape
            x2 = x2.reshape(B2 * T2_new, C2_new, H2_new, W2_new)
            
        # 最终转换为 5D 张量返回
        B_final, C_final, H_final, W_final = x.shape
        T_final = B_final // B
        x = x.reshape(B, T_final, H_final, W_final, C_final)
        
        B2_final, C2_final, H2_final, W2_final = x2.shape
        T2_final = B2_final // B2
        x2 = x2.reshape(B2, T2_final, H2_final, W2_final, C2_final)
...
class CuboidTransformerModel(nn.Module):
    ...
    Cuboid Transformer 用于时空预测任务的主模型。
    该模型采用非自回归的Encoder-Decoder结构，核心创新为Cuboid Attention（立方体注意力），
    能高效建模大规模时空依赖，适用于气象、遥感等时空序列预测。
    主要流程：
    1. 初始下采样（Initial Encoder）：对输入数据降维，支持多模态输入（如vil、lght等）。
    2. 位置编码（PosEmbed）：为时空特征添加可学习的位置编码。
    3. 编码器（Encoder）：多层Cuboid Attention + PatchMerging3D下采样，提取多尺度时空特征。
    4. 解码器（Decoder）：多层Cuboid Attention + Upsample3DLayer上采样，逐步恢复空间分辨率。
    5. 最终上采样与卷积（Final Decoder）：进一步上采样和卷积处理。
    6. 输出投影（dec_final_proj）：将特征映射到目标通道数，得到最终预测结果。
    支持全局向量（global vectors）机制，提升全局时空状态建模能力。
    支持灵活的下采样/上采样方式、激活函数、归一化、注意力模式等参数配置。
    """
    def __init__(self,
                 input_shape,
                 target_shape,
        ...
        参数说明：
        input_shape: 输入数据的形状 (T, H, W, C_in)
        target_shape: 目标输出的形状 (T_out, H, W, C_out)
        base_units: 基础通道数
        block_units: 每个block的通道数列表
        scale_alpha: 通道数缩放因子
        num_heads: 注意力头数
        attn_drop, proj_drop, ffn_drop: 各类dropout
        downsample, upsample_type: 下采样/上采样方式
        enc_depth, dec_depth: 编码器/解码器每层深度
        enc_attn_patterns, dec_self_attn_patterns: 每层注意力模式
        ...（其余参数详见源码注释）
        """
        ...
        if self.num_global_vectors > 0:
            # 可学习的全局向量参数，提升全局时空建模能力
            self.init_global_vectors = nn.Parameter( # torch.Size([8, 64])
    ...
    def get_initial_encoder_final_decoder(
        ...
        """
        构建初始下采样（InitialEncoder）和最终上采样（FinalDecoder）模块。
        支持 conv 和 stack_conv 两种方式。
        返回下采样后的新输入形状。
        """
    ...
    def reset_parameters(self):
        """
        参数初始化，包括全局向量、初始下采样、最终上采样、编码器、解码器等。
        """
        if self.num_global_vectors > 0:
    ...
    def get_initial_z(self, final_mem, T_out):
        """
        根据编码器最后一层输出，初始化解码器的输入z。
        支持多种初始化方式：全零、最近邻插值、最后一帧、均值。
        """
        B = final_mem.shape[0]
    ...
    def forward(self, x,x2, verbose=False):
        """
        ...
        # 数据初步处理，encoder对原始输入（支持多模态，如vil和lght）做初步特征提取和空间下采样，
        # 提升特征表达能力，并降低后续计算量。

        # x torch.Size([2, 13, 384, 384, 1])
        # x2 torch.Size([2, 13, 384, 384, 1])
        x, x2 = self.initial_encoder(x, x2) # [2,13,32,32,128]


        # 位置编码
        x, x2 = self.enc_pos_embed(x, x2) # [2,13,32,32,128]


        # 全局向量初始化
        if self.num_global_vectors > 0: # 走了这里
            init_global_vectors = self.init_global_vectors\
                .expand(B, self.num_global_vectors, self.global_dim_ratio*self.base_units) # torch.Size([8, 64])->torch.Size([1, 8, 64])


            # 对initial_encoder输出的特征做深层时空建模，
            # 提取多尺度时空特征（多层Cuboid Attention+PatchMerging）
            mem_l, mem_l2, mem_global_vector_l = self.encoder(x, x2, init_global_vectors)  
            # mem_l: len=2, torch.Size([2, 13, 32, 32, 128]), torch.Size([2, 13, 16, 16, 256]) ;
            # mem_l2: len=2, torch.Size([2, 13, 32, 32, 128]), torch.Size([2, 13, 16, 16, 256]) ;
            # mem_global_vector_l: len=2, [2,8,128],[2,8,256]; 
        else: # 没经过这里
            mem_l, mem_l2 = self.encoder(x, x2)
        ...
        # 作用：生成解码器的初始输入 z，即解码器的"起点"特征。
        # 原理：根据编码器最后一层的输出（最深层的时空特征），用插值/均值/复制等方式生成目标长度的初始特征序列，作为解码器的输入。
        initial_z = self.get_initial_z(final_mem=mem_l[-1], T_out=T_out) # torch.Size([2, 12, 16, 16, 256])
        if self.num_global_vectors > 0: # 也走这里
            # 解码器
            # 作用：主解码器，逐步上采样并融合编码器多层特征，恢复空间分辨率，生成高层次时空特征。
            # 实现：CuboidTransformerDecoder，堆叠多层Cuboid Attention+Upsample，支持跨层特征融合（skip connection）、全局向量机制等。
            dec_out = self.decoder(initial_z, mem_l, mem_l2, mem_global_vector_l)
        else:
            dec_out = self.decoder(initial_z, mem_l, mem_l2)
        # 进一步细化解码器，作用：最后的上采样和卷积细化，进一步提升空间分辨率和特征表达，准备输出。
        dec_out = self.final_decoder(dec_out) # [2,12,384,384,16]
        # 映射
        out = self.dec_final_proj(dec_out) # torch.Size([2, 12, 384, 384, 1])
        return out
